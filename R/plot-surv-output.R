...
plot_surv_output = function(data_KM, sim_data, rate_str = NULL, leg.x = 250, leg.y = 1){
  ...
  if(any(is(sim_data) == "survfit")){
    lines(sim_data, conf.int = F, col = "red", mark.time = F, lty = 1)
    ...
    for(i in 1:length(rates)){
      ...
      temp_surv = survival::survfit(survival::Surv(time = detect_time, event = infected) ~ 1, data = sub_sim_data)
      lines(temp_surv, conf.int = F, col = plot_col[i], mark.time = F)
